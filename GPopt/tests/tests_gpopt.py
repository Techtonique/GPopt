...
class TestBranin(ut.TestCase):
    ...
    def test_base_and_more_iter(self):
        ...
        def braninsc(x):
            x1_bar = 15*x[0] - 5
            x2_bar = 15*x[1]
            term1 = (x2_bar - (5.1/(4*np.pi**2)) * x1_bar**2 + (5/np.pi)*x1_bar - 6)**2
            term2 = 10*(1-1/(8*np.pi))*np.cos(x1_bar)
            
            return((term1 + term2 - 44.81) / 51.95)
        
        gp_opt1 = gp.GPOpt(objective_func=braninsc, 
                       lower_bound = np.array([-5, 0]), 
                       upper_bound = np.array([10, 15]), 
                       n_choices=10000, n_init=10, n_iter=50)   
    ...
    def test_early_stopping(self):
        ...
        gp_opt1 = gp.GPOpt(objective_func=braninsc, 
                   lower_bound = np.array([-5, 0]), 
                   upper_bound = np.array([10, 15]), 
                   n_choices=10000, n_init=10, n_iter=200)        
        ...
        gp_opt2 = gp.GPOpt(objective_func=braninsc, 
                   lower_bound = np.array([-5, 0]), 
                   upper_bound = np.array([10, 15]), 
                   n_choices=10000, n_init=10, n_iter=30)        
        gp_opt2.optimize()
    ...
    def test_save(self):
        ...
        gp_opt1 = gp.GPOpt(objective_func=braninsc, 
                       lower_bound = np.array([-5, 0]), 
                       upper_bound = np.array([10, 15]), 
                       n_choices=10000, n_init=10, n_iter=25, 
                       save = path)        
        gp_opt1.optimize(verbose=1)
        gp_opt1.close_shelve()
        
        gp_optload = gp.GPOpt(objective_func=braninsc, 
                       lower_bound = np.array([-5, 0]), 
                       upper_bound = np.array([10, 15]))
