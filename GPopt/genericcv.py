...
class MLOptimizer:
    ...
    def optimize(
        self, X_train, y_train, estimator_class, param_config, verbose=2
    ):
        """
        ...
        def crossval_objective(x):
            ...
            for i, param_name in enumerate(param_names):
                ...
                if dtypes[param_name] == "int":
                    value = int(
                        np.round(value)
                    )  # Use round instead of ceil for better behavior
        ...
        try:
            self.optimization_result = gp_opt.optimize(
                verbose=verbose, abs_tol=1e-4
            )
            ...
            print(
                "Debugging: Testing objective function with initial points..."
            )
    ...
    def get_best_score(self):
        ...
        return (
            -self.optimization_result.best_score
    ...
    def create_optimized_estimator(self):
        ...
        if self.estimator_class is None:
            raise ValueError(
                "No estimator class stored. Call optimize() first."
            )

        if self.param_config is None:
            raise ValueError(
                "No parameter configuration stored. Call optimize() first."
            )
