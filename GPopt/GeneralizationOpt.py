...
class GenericSurrogate(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y, cal_ratio=0.5):
        """
    ...
    def predict(self, X, return_std=False, confidence=0.95):
        ...
        if self.conformal and self.nonconformity_scores_ is not None:
            ...
            quantile_level = min(
                1.0, np.ceil((1 - alpha) * (n_scores + 1)) / n_scores
            )

            q = np.quantile(self.nonconformity_scores_, quantile_level)
            y_lower = y_pred - q
            y_upper = y_pred + q
            ...
            try:
                # For GP-like models
                if (
                    hasattr(self.base_model, "predict")
                    and "return_std"
                    in self.base_model.predict.__code__.co_varnames
                ):
                    _, y_std = self.base_model.predict(X, return_std=True)
                    y_lower = y_pred - 1.96 * y_std  # 95% CI for Gaussian
...
class GeneralizationOpt:
    ...
    def _denormalize_config(self, normalized_config):
        ...
        for i, param_name in enumerate(self.hyperparam_names):
            ...
            if log_scale:
                ...
                config[param_name] = np.exp(
                    log_min + norm_val * (log_max - log_min)
                )
            else:
    ...
    n_folds=5,
        ...
        FIXED: Now uses train/val split to avoid test set leakage
        - CV on inner training set → cv_train
        - Evaluation on validation set → val_score
        - Gap = cv_train - val_score
        - X_test/y_test reserved for final evaluation only
        """
        ...
        # Validation set is used to compute the gap (not test set!)
        if len(np.unique(y_train)) < 20:  # Classification
            ...
            evaluation_fn = evaluation_fn or balanced_accuracy_score
            ...
            evaluation_fn = evaluation_fn or r2_score
            kfold = KFold(
                n_splits=n_folds,
                shuffle=True,
                random_state=self.random_state,
        ...
        print(
            f"Evaluating {n_samples} configurations ({n_folds}-fold CV + validation)..."
        )
        ...
        for idx, norm_config in tqdm(enumerate(self.sobol_samples)):
            if (idx + 1) % 50 == 0:
                print(
                    f"  Progress: {idx + 1}/{n_samples} "
                    f"({100*(idx+1)/n_samples:.1f}%)"
                )

            config = self._denormalize_config(norm_config)

            # Cross-validation score on inner training set
            cv_scores = []
            for train_idx, val_idx in kfold.split(X_tr, y_tr):
                if hasattr(X_tr, "iloc"):  # DataFrame
                    X_cv_tr, X_cv_val = X_tr.iloc[train_idx], X_tr.iloc[val_idx]
                    y_cv_tr, y_cv_val = y_tr.iloc[train_idx], y_tr.iloc[val_idx]
                else:  # ndarray
                ...
                cv_scores.append(evaluation_fn(y_cv_val, y_pred))

            cv_train = np.mean(cv_scores)

            # Validation score (this is what we use for the gap!)
            model = model_class(**config, **model_kwargs)
            model.fit(X_tr, y_tr)
            y_pred_val = model.predict(X_val)
            val_score = evaluation_fn(y_val, y_pred_val)

            # Generalization gaps (CV train vs validation, NOT test!)
            gap_abs = cv_train - val_score
            gap_norm = (
                (cv_train - val_score) / (1 - cv_train) if cv_train < 1 else 0
            )
            gap_mpe = (
                ((cv_train - val_score) / cv_train * 100)
                if cv_train > 0
                else 0
            )
        ...
        print(
            f"  CV Train Score: {self.evaluation_results['cv_train'].mean():.4f} ± "
            f"{self.evaluation_results['cv_train'].std():.4f}"
        )
        print(
            f"  Validation Score: {self.evaluation_results['val_score'].mean():.4f} ± "
            f"{self.evaluation_results['val_score'].std():.4f}"
        )
        print(
            f"  Absolute Gap: {self.evaluation_results['gap_abs'].mean():.4f} ± "
    ...
    def fit_surrogate(self, surrogate, target_gap="gap_abs"):
        ...
        print(
            f"Training surrogate on {len(X_train)} samples, "
    ...
    def optimize_acquisition(
        ...
        for param, value in best_config.items():
            print(
                f"    {param}: {value if isinstance(value, int) else f'{value:.4f}'}"
            )
    ...
    def run_diagnostics(
        ...
        eval_results = self.evaluate_configurations(
            X_train, X_test, y_train, y_test, model_class, model_kwargs
        )
